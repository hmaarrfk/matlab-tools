classdef InfiniiVision2000 < handle
  ...
  methods
    function self = InfiniiVision2000(ManufacturerID_hex, ModelCode_hex, SerialNumber_str, InterfaceNumber_str)
      ...
      % 
      % To call the 4 connection scope
      % scope = InfiniiVision2000(['0x', dec2hex(2391, 4)], ['0x', dec2hex(6052, 4)], 'MY52163723');
      if nargin < 1
        ManufacturerID_hex = ['0x', dec2hex(2391, 4)];
      end
      if nargin < 2
        ModelCode_hex = ['0x', dec2hex(6041, 4)];
      end
      if nargin < 3
        SerialNumber_str = 'MY52011887';
      end
      if nargin < 4
        InterfaceNumber_str = num2str(0);
      end
      self.usb_str_name = ...
    ...
    function err = check_instrument_errors(self)
      ...
      if strcmp(err_str(end), char(10)) || strcmp(err_str(end), char(13))
